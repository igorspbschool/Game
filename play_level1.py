...
from victory_prom import main as victory_prom_main
...
class Game():
    ...
    def update_hero(self):
        ...
        # print(time_game)
        if self.reiting == 30:
            itog_time_reitig = self.reiting
            if time_game < 40:
                itog_time_reitig += 20
            elif time_game > 40 and time_game < 50:
                itog_time_reitig += 10
            else:
                itog_time_reitig += 5
            pygame.quit()
            # Промежуточн рез между уровнями
            victory_prom_main(itog_time_reitig)
            terminate()
    ...
    def spawn_enemy(self):
        ...
        if pygame.time.get_ticks() // 1000 > 15 and self.amount_enemy < 10:
...
def main(reiting):
    ...
    with open(f"{MAPS_DIR}/map.txt") as input_file:
        map_for_zoloto = []
        for line in input_file:
